<?php 
	...
	if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
		$num = dbRs(sprintf("SELECT count(1) FROM ec_pages WHERE id = %s AND wid = $worldid", GetSQLValueString($_POST['frameid'], "int")));
		if($num > 1){
	...
	$row_EditHistory = dbRow($query_EditHistory);
	
	if(!$row_EditHistory){
		screenMessage("Error","This page has not been created yet.");
	}
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<h3>Endless Choice &gt;&gt; Editing &gt;&gt; World ID: <?php echo $worldid; ?>&gt;&gt; FrameID: <?php echo floatval($id) ?></h3>
		...
		<form action="<?php echo $editFormAction; ?>" id="form1" name="form1" method="POST" onsubmit="">
			...
			<fieldset>
				...
				<div style="padding:1px">
					<textarea name="content" style="height:300px;resize:none" id="content" wrap="virtual"><?php echo htmlspecialchars($row_EditHistory['content']); ?></textarea>
				</div>
				<div style="padding:1px">
					<input type="button" name="script" value="script標記" onclick="replaceit('script');" />
					<input type="button" name="span" value="SPAN" onclick="replaceit('span');" />
					<input type="button" name="span" value="變數容器" onclick="replaceit('var');" />
			...
			<fieldset>
				...
				<div id="js" <?php if($row_EditHistory['js']==""){?>style="display:none"<?php } ?>>
					...
					<input type="button" name="ec_add" value="ec_add('設定變數',數值)" onclick="replaceit2('ec_add');" />
					<input type="button" name="ec_push" value="ec_push('容器id', 變數)" onclick="replaceit2('ec_push');" />
					<input type="button" name="ec_dice" value="ec_dice(100)" onclick="replaceit2('ec_dice');" />
					...
					<input type="button" name="ec_setTitle" value="ec_setTitle('頁面標題')" onclick="replaceit2('ec_setTitle');" />
					<input type="button" name="ecPreviousButtonId" value="上一頁選擇" onclick="replaceit2('ecPreviousButtonId');" />
					<input type='button' name="ec_hideOption" value="ec_hideOption(隱藏選項)" onclick="replaceit2('ec_hideOption');" />
					<br />
					<textarea name="js" id="jsf" wrap="virtual" style="height:150px" ><?php echo htmlspecialchars($row_EditHistory['js']); ?></textarea><br />
			...
			<fieldset>
				...
				<table width="590">
					...
					<tr>
						...
						<td><input name="c6to" type="text" id="c6to" value="<?=$row_EditHistory['c6to']; ?>" size="20" />
						<a href="editfile.php?worldid=<?php echo $worldid; ?>&amp;id=<?=$row_EditHistory['c6to']; ?>"><img src="../img/green-arrow-right.gif" /></a> </td>
					</tr>
				</table>
			</fieldset>
			<br />
			<fieldset>
				<legend>全頁特效</legend>
				*全頁打字機特效速度(0為關閉此特效):
				<input name="typing" type="text" id="typing" value="<?=$row_EditHistory['typing']; ?>" size="7" maxlength="4" />
				ms/w(愈小愈快, 建議50 - 150)
				<br />
				<br />
				*此特效不能和變數功能在同一頁共用
			...
			<br /><br />
            <?if(!$_COOKIE['EC_READONLY']){?><input type="submit" name="Submit" id="Submit" value="Submit" style="margin-left:10px" /><?}?>
