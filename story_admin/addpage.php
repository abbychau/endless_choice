<?php 
	...
	if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
		...
		$chk = sprintf("SELECT count(1) FROM ec_pages WHERE id = ? AND wid= ?",[$_POST['frameid'],$worldid]);
		
		if(dbRs($chk) > 0){
			die("Duplicated Frame ID ".$_POST['frameid']);
		}
		
		dbQuery("INSERT INTO ec_pages (wid, id, title, content, c1, c2, c3, c4, c5, c6, c1to, c2to, c3to, c4to, c5to, c6to, js, typing) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
		[$worldid,
